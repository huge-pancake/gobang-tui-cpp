// From https://github.com/ShakaUVM/colors

#ifndef __COLORS_H__
#define __COLORS_H__
// colors.h - A lightweight replacement for NCURSES that does 24 bit color,
// cursor movement, and nonblocking I/O It uses standard ANSI escape codes to
// set the foreground and background text colors for a terminal Prerequisite:
...
// Example: resetcolor();
inline void resetcolor() { std::cerr << "\033[0m"; }

// These functions are like a mini NCURSES library

// Returns the ROWS and COLS of the current terminal
// Example: auto [rows,cols] = get_terminal_size() will make a variable named
// rows that has the number of rows of the current screen, and cols with the
// number of columns
...
// Example: movecursor(10,40) will move the cursor to the row 10 down from the
// top and 70 to the right of the left edge
...
// Clears the screen
// Example: clearscreen();
inline void clearscreen() { std::cerr << "\033[2J"; }
...
// This function changes the standard input from "canonical" mode (which means
// it buffers until a newline is read) into raw mode, where it'll return one
// keystroke at a time Example: set_raw_mode(true) will turn on nonblocking I/O
// for cin Example: set_raw_mode(false) will reset I/O to work like normal
...
// Example: int bytes_available = kbhit() will return how many bytes are in the
// input queue to be read
inline int kbhit() {
...
// These might change, so be prepared to just deal with raw numbers from
// quick_read
const int ERR = -1;
...
// TODO: Improve escape sequence parsing, especially if two are hit between
// polls Does a non-blocking I/O read from standard input, returns one keystroke
// Lightweight Equivalent to NCURSES getch()
// Precondition: Requires set_raw_mode(true) to work
// Example: int ch = quick_read() will return ERR if no key has been hit, or 'A'
// if they hit A, or PAGE_DOWN if they hit page down, etc.
